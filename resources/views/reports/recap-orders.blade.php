...
<head>
    ...
    <title>Recap Orders Report: {{$str_period}}</title>
    <style>
        h1, h2, h3 {
            text-align: center;
        }
...
<h2>Period: {{$str_period}}</h2>
@foreach($types as $type)
    @php
        $service_ids = \App\Models\layanan::where('jenis_id', $type->id)->pluck('id')->toArray();
        $orders = \App\Models\Pemesanan::whereIn('layanan_id', $service_ids)
            ->whereYear('created_at', substr($period, -4))->whereMonth('created_at', substr($period, 0, 2))->get();
    @endphp
    <table id="data-table">
        <thead>
        <tr>
            <th colspan="5">Service Type: <strong style="text-transform: uppercase">{{$type->nama}}</strong></th>
        </tr>
        <tr>
            <th align="center">#</th>
